...
// Función que incicializa el tablero en "-"
void initializeBoard(char tablero[TAMANO_TABLERO][TAMANO_TABLERO])
{
    for (int i = 0; i < TAMANO_TABLERO; i++) //Recorre las filas del tablero
    {
        for (int j = 0; j < TAMANO_TABLERO; j++) //Recorre las columnas del tablero
        {
            tablero[i][j] = '-';
        }
    }
...
// Función para que dispare la PC
void PCTurn(int tamanio, char tableroJugador[tamanio][tamanio], int *barcosHundidos, int *barcosRestantes)
{
    ...
    while (validarDisparo(tableroJugador, randomRowPosition, randomColumnPosition) == 0)
    {
...
int validateShot(char tablero[TAMANO_TABLERO][TAMANO_TABLERO], int x, int y) {
...
void solicitarNombre(char nombre[]); Listo!!
void mostrarReglas(); Listo!!
void inicializarTablero(char tablero[TAMANO_TABLERO][TAMANO_TABLERO]); Listo!!
